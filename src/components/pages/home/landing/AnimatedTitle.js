import React from 'react';

import styled, { keyframes } from 'styled-components';

const drawLine = keyframes`
  25% {
    fill-opacity: 0;
  }

  60% {
    fill-opacity: 1;
  }

  100% {
    fill-opacity: 1;
    stroke-dashoffset: 0;
  }
`;

const StyledSVG = styled.svg`
  max-width: 1000px;
  stroke-width: 2;
  width: 100%;
  path {
    stroke: ${props => props.theme.color};
    fill: ${props => props.theme.color};
    animation: 4s linear forwards ${drawLine};
    fill-opacity: 0;
    stroke-dasharray: 1500;
    stroke-dashoffset: 1500;
  }
`;

const AnimatedTitle = () => {
  return (
    <StyledSVG xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1356.8 181.5">
      <title>Joel Hoelting</title>
      <path
        id="J"
        d="M89.4,119.5c0,44.4-36.5,64.7-82.4,64.7V162.6c33.9,0,60.9-13.5,60.9-43.1V56.3H27.5V34.9H89.4Z"
        transform="translate(-5.5 -6.1)"
      />
      <path
        id="O"
        d="M120.2,55.4a75.5,75.5,0,0,1,107.8,0,76.8,76.8,0,0,1,0,108,75.5,75.5,0,0,1-107.8,0,76.8,76.8,0,0,1,0-108Zm92.5,15.4A55,55,0,0,0,174,54.6a53.6,53.6,0,0,0-38.2,16.2,54.6,54.6,0,0,0,0,77.3A53.6,53.6,0,0,0,174,164.3a55,55,0,0,0,38.6-93.5Z"
        transform="translate(-5.5 -6.1)"
      />
      <path
        id="E"
        d="M284.6,162.6H354v21.6H262.8V34.9h91V56.5H284.6V98.5h67.7v21.8H284.6Z"
        transform="translate(-5.5 -6.1)"
      />
      <path id="L" d="M386.2,162.6h70.2v21.6H364.7V34.9h21.6Z" transform="translate(-5.5 -6.1)" />
      <path
        id="H"
        d="M562.2,97.9h63.2v-63h21.8V184.2H625.4V119.3H562.2v64.9H540.6V34.9h21.6Z"
        transform="translate(-5.5 -6.1)"
      />
      <path
        id="O-2"
        data-name="O"
        d="M657.2,109.4a77.6,77.6,0,0,1,22.2-54,75.5,75.5,0,0,1,107.8,0,76.8,76.8,0,0,1,0,108,75.5,75.5,0,0,1-107.8,0A77.6,77.6,0,0,1,657.2,109.4Zm130.5,0a54.7,54.7,0,0,0-54.4-54.9A53.6,53.6,0,0,0,695,70.8a54.6,54.6,0,0,0,0,77.3,53.6,53.6,0,0,0,38.2,16.2,54.7,54.7,0,0,0,54.4-54.9ZM714.4,7.6c-6.6,0-11.7,4.3-11.7,10.7s5.1,10.2,11.7,10.2,12.4-5.1,12.4-10.2S721.7,7.6,714.4,7.6Zm39.3,0c-6.6,0-12,4.3-12,10.7s5.3,10.2,12,10.2,12.4-5.1,12.4-10.2S761.2,7.6,753.7,7.6Z"
        transform="translate(-5.5 -6.1)"
      />
      <path
        id="L-2"
        data-name="L"
        d="M845.5,162.6h70.2v21.6H823.9V34.9h21.6Z"
        transform="translate(-5.5 -6.1)"
      />
      <path
        id="T"
        d="M933.7,184.8V54.8H888.4V34.7h111.9V54.8H955.3v130Z"
        transform="translate(-5.5 -6.1)"
      />
      <path
        id="I"
        d="M1084.9,56.3h-27.8V162.6h27.8v21.6h-77.1V162.6h27.8V56.3h-27.8V34.9h77.1Z"
        transform="translate(-5.5 -6.1)"
      />
      <path
        id="N"
        d="M1094.5,184.2V34.5l16.7.2L1183.9,140V34.7h21.4V184.2h-17.1L1116,79.8V184.2Z"
        transform="translate(-5.5 -6.1)"
      />
      <path
        id="G"
        d="M1336.6,88.3c-6-24.1-29-32.9-48-32.9-29.3,0-51.9,22.8-51.9,54.2s22.6,54.9,51.9,54.9c18.4,0,38-8.5,47-30.3h-55.3V112.6h80.5c-1.1,12.6-2.1,20.9-4.1,25.8v-.2c-10.2,28.6-41,47-68.1,47-41.8,0-73.4-31.6-73.4-75.6s32.2-75.8,73.4-75.8c28.6,0,63.2,14.7,71.1,54.4Z"
        transform="translate(-5.5 -6.1)"
      />
    </StyledSVG>
  );
};

export default AnimatedTitle;
